%function quantizer1
%use of psychoacoustic model and adaptable quantizer

function[x_rec,B_total]=adaptable_quantizer(x_framed,L,M,fs,Tg,length_x) 
    ...
    for num_frame=1:frames
        for k=1:M
            Tg_min(:,k,num_frame)=min(Tg_framed(:,k,num_frame)); %find the min Tg of each filter in each frame
            B(k,num_frame)=ceil(log2(R./Tg_min(:,k,num_frame))-1); %bits/sample
        end
    end
